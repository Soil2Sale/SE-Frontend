...
export default function BuyerOffersPage() {
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError(
        (err as { message?: string })?.message || "Failed to update offer.",
      );
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-3xl p-6 shadow-sm">
        ...
        <p className="text-gray-600">
          Track, update, or withdraw your crop purchase offers
        </p>
      </div>
              ...
              >
                <div className="px-3 py-2">
                  ...
                  <div className="flex items-center gap-2 mt-0.5 text-[11px] text-gray-500">
                    <span>
                      Ask: {offer.crop_listing?.expected_price ?? "—"}/kg
                    </span>
                    <span>•</span>
            ...
            <div className="bg-[#1a4d2e] px-8 py-6 flex items-center justify-between">
              <div>
                ...
                <p className="text-green-200 text-sm mt-0.5">
                  {updateTarget.crop_listing?.crop_name ?? "Crop"} ·{" "}
                  {updateTarget.crop_listing?.quality_grade ?? ""}
                </p>
              </div>
              <button
                onClick={closeModal}
              ...
              <div className="grid grid-cols-2 gap-3 text-center">
                <div>
                  ...
                  <p className="font-bold text-[#1a4d2e] text-sm">
                    {updateTarget.offered_price}/kg
                  </p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 mb-0.5">Asking Price</p>
                  <p className="font-bold text-gray-900 text-sm">
                    {updateTarget.crop_listing?.expected_price
                      ? updateTarget.crop_listing.expected_price
                      : "—"}
                    /kg
                  </p>
              ...
              <div>
                ...
                {predicting && (
                  <div className="mt-2 flex items-center gap-2 text-sm text-gray-500">
                    <Loader2 className="w-4 h-4 animate-spin" /> Fetching AI
                    price suggestion...
                  </div>
                )}
