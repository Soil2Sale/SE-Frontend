...
export default function MarketplacePage() {
  ...
  const [selectedListing, setSelectedListing] =
    React.useState<CropListing | null>(null);
  ...
  const openOfferModal = async (listing: CropListing) => {
    ...
    try {
      const res = await axios.post(
        `http://127.0.0.1:8000/predict/${listing.crop_name}`,
        {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg =
        (err as { message?: string })?.message || "Failed to submit offer.";
      setSubmitError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  const avgPrice =
    activeListings.length > 0
      ? activeListings.reduce((sum, l) => sum + l.expected_price, 0) /
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm">
        <div>
          ...
          <p className="text-gray-600">
            Browse available crop listings and make offers directly to farmers
          </p>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">
              No crop listings available right now
            </p>
          </div>
        ) : (
          <div className="flex justify-center">
            <AnimatedList
              ...
              onItemSelect={(_item, index) => setDetailListing(listings[index])}
              showGradients={false}
            ...
            <div className="bg-[#1a4d2e] px-6 py-5 flex items-center justify-between">
              <div>
                ...
                <p className="text-green-200 text-sm mt-0.5">
                  Listed on {formatDate(detailListing.created_at)}
                </p>
              </div>
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="bg-[#1a4d2e] px-8 py-6 flex items-center justify-between">
              <div>
                ...
                <p className="text-green-200 text-sm mt-0.5">
                  {selectedListing.crop_name} · {selectedListing.quality_grade}
                </p>
              </div>
              <button
                onClick={closeModal}
            ...
            <div className="mx-8 mt-6 p-4 bg-gray-50 rounded-2xl border border-gray-100">
              <div className="grid grid-cols-3 gap-3 text-center">
                ...
                <div>
                  <p className="text-xs text-gray-500 mb-0.5">Total Value</p>
                  <p className="font-bold text-gray-900 text-sm">
                    {selectedListing.quantity * selectedListing.expected_price}
                  </p>
                </div>
              </div>
            </div>
